...
// input an angle in radians, and track in absolute rotation. if the new angle
// is more that 2pi from the old angle, it will be wrapped around to the other
// side of the circle.
class AngleTracker {
  float angle;
...
namespace math {
namespace detail {
template <typename Real> constexpr Real wrap(Real x) {
...
template <typename Real> constexpr Real sin(Real x) {
  return detail::wrap(x) /
         (1 + detail::sqr(detail::wrap(x)) /
                  detail::sin_cfrac(detail::sqr(detail::wrap(x))));
}
template <typename Real> constexpr Real cos(Real x) {
...
class Executor {
  template <class Callable> constexpr void add(Callable c) {
...
private:
  ...
  static constexpr uint32_t m_period =
      42000 / 40; // divide by desired frequency in khz
public:
  ...
  template <class Numeric> constexpr Numeric truncate1(Numeric value) {
  ...
  void updateCurrentAngle(float _a, float _b, float motorTorque) {
    ...
    // setabc(m_thetaCos*motorTorque, m_thetaCos*motorTorque,
    // motorTorque*new_theta);
  }
  void enable_output() {
    // m_period
  ...
  void update_from_phasePQ(Numeric d, Numeric q) {
    // todo update to constexpr function

#define USE_LOCAL_TRANSFORMS 1
#if USE_LOCAL_TRANSFORMS // trying to make this faster.

    /* Calculate pIalpha using the equation, pIalpha = Id * cosVal - Iq * sinVal
     */
    const auto alpha = d * m_thetaCos - q * m_thetaSin;
    /* Calculate pIbeta using the equation, pIbeta = Id * sinVal + Iq * cosVal
     */
    //            *pIbeta = Id * sinVal + Iq * cosVal;
    const auto beta = d * m_thetaSin + q * m_thetaCos;
    // inverse clark transform converts 90 degree phase alpha, beta to three
    // phase square root of 3 is 1.732050807568877
  ...
  static constexpr Numeric envelope(Numeric input, Numeric lowerLimit,
                                    Numeric upperLimit) {
  ...
  // template<typename Numeric>
  // using Numeric = float;
  constexpr void setPhase(const float d, const float theta) {
    const auto mySin = math::sin(theta);
    const auto myCos = math::cos(theta);
    const auto alpha = d * myCos;
    const auto beta = d * mySin;
    // inverse clark transform converts 90 degree phase alpha, beta to three
    // phase square root of 3 is 1.732050807568877
    constexpr auto kSqr3Over2 = 1.732050807568877 / 2;
    const auto a = alpha;
    const auto b = -1 / 2 * alpha + kSqr3Over2 * beta;
    const auto c = -1 / 2 * alpha - kSqr3Over2 * beta;
    setabc(a, b, c);
  }
...
public:
  ADCDriver(ADC_HandleTypeDef hAdc) : m_hadc{hAdc} {
...
public:
  ...
  NokiaLcd(SPI_HandleTypeDef spi) : m_hspi(spi) {
    // init spi
    HAL_SPI_Init(&m_hspi);
  }
  // output
...
class AdcDriver {
  ...
  // this is a hack to get around the fact that function pointers are needed to
  // be passed to HAL conversion complete
private:
  ADC_HandleTypeDef m_hadc;
  // buffer to store adc values
  std::array<uint32_t, 8> m_adc_buffer;
...
public:
  //    AdcDriver(ADC_HandleTypeDef hAdc) : m_hadc
  //    {
  //        //        ADC_HandleTypeDef hAdc
  //        //        m_hadc.DMA_Handle->Init.
  //    }
...
[[noreturn]] void cppmain(struct p_HW hw) {
  ...
  pwm4.enable_output();
  pwm2.setabc(0, 0, 0);
  // set initial phase to be updated by the adc filter in
  // a future update.
  pwm1.setPhase(0, 0);
  pwm2.setPhase(0, 0);
  pwm3.setPhase(0, 0);
...
//    std::array<uint32_t, 8> adc_buffer = {1, 2, 3};
    ...
    for (auto i = 0U; i < currentSignal.size(); i++) {
      for (auto stepPerSample = 0; stepPerSample < 3; stepPerSample++) {
        ...
        const auto b_signal =
            currentSignal.at((i + phase_offset) % currentSignal.size());
        const auto c_signal = currentSignal.at(
            (i + phase_offset + phase_offset) % currentSignal.size());
        ...
        // update led before and after delay to find how much processing time we
        // are spending.
        // ledPattern();
        // HAL_Delay(0);
        ledPattern();
